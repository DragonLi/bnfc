...
csharptest namespace cf = unlines [
  ...
  "        parser.Context = new ParsingContext();",
  "        try",
  "        {",
  "          var parse_tree = parser.Parse" ++ def ++ "();",
  "          if (parse_tree != null)",
  "          {",
  ...
  "            Console.Out.WriteLine(\"unexpected parsed tree:{0}\", PrettyPrinter.Show(parser.CurrentValue,parser.CurrentValueTag));",
  "          }",
  "        }",
  "        catch(Exception e)",
  "        {",
  "          Console.Out.WriteLine(\"Parse NOT Successful:\");",
  "          Console.Out.WriteLine(parser.LastErrorMsg);",
  "          Console.Out.WriteLine(parser.ScannerError);",
