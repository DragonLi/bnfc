...
header :: Namespace -> CF -> SymEnv -> String
header namespace cf env = unlines [
  "/* This GPPG file was machine-generated by BNFC */",
  "",
  "%ScanBuffType Scanner",
  "%namespace " ++ namespace,
  "%{",
  ...
  where
    ...
    defineHintMethods = unlines[
      ...
      "",
      "   public string LastErrorMsg { get { return HintAt(Scanner.yyline, Scanner.yycol, Scanner.yypos); } }",
      "",
      "   public string HintAt(LexLocation loc){return HintAt(loc.StartLine, loc.StartColumn, loc.StartIndex);}",
      "",
      "   public string HintAt(int tokLin, int tokCol, int tokPos)",
      "   {",
...
declarations cf = unlinesInline $ map (typeNT cf) (positionCats cf ++ allCats cf)
 where --don't define internal rules
   typeNT cf nt | (isPositionCat cf nt || rulesForCat cf nt /= []) = "%type <" ++ (varName (show$normCat nt)) ++ "> " ++ (identCat nt)
   typeNT _ _ = ""
